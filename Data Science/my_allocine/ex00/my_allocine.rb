...
requests["Find the actors with all information who played a role in the movies 'Annie Hall'"] = "SELECT * FROM actors WHERE id IN (SELECT act_id FROM movies_actors INNER JOIN movies ON movies_actors.mov_id = movies.id WHERE mov_title = 'Annie Hall');"
...
WHERE NOT EXISTS (
    ...
    FROM movies_actors
    JOIN movies ON movies_actors.mov_id = movies.id
    WHERE movies_actors.act_id = actors.id
    AND CAST(strftime('%Y', mov_dt_rel) AS INTEGER) BETWEEN 1990 AND 2000
)
"""
